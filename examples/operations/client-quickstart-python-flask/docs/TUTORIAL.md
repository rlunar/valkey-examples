...
You will build the complete quickstart capsule, including every application,
container, lifecycle, and test file you need to run:
...
uv generates the lockfile. You can find the README and the three documents in
`docs/`; this tutorial does not reproduce them inline.

## Read this first

For a first pass, focus on sections 4 and 5, then run section 13. Those parts
show the connection class, Flask route, and working application. Sections 8
through 12 explain containers, scripts, and tests.

The main request path is:

```text
POST /value -> SET -> GET /value -> GET -> JSON response
```

Key words:

- **route:** a URL handled by a Python function;
- **JSON:** text used to send structured data over HTTP;
- **primary:** the Valkey node that accepts writes;
- **replica:** a node that copies the primary's data; and
- **cluster:** Valkey nodes that split the keys between them.
...
1. `load_dotenv()` loads local values before `ValkeyClient()` creates the
   connection object.
...
## 9. Understand the shared Compose topology

The manifest points to [`infra/compose.yaml`](../../../../infra/compose.yaml)
for `valkey-standalone-replicated` and `valkey-cluster-6`. The local
[`compose.yaml`](../compose.yaml) contains only the Flask services.

The following expanded blocks document the topology that is now implemented
once in the shared infrastructure capsule. Do not append them to the local
file.
...
## 10. Add thin lifecycle adapters

### 10.1 Shared shell behavior

The checked-in [`scripts/common.sh`](../scripts/common.sh) maps `TOPOLOGY` to
the declared shared profile and sources `../../../infra/scripts/capsule.sh`.
The expanded listing below explains the behavior that was extracted; do not
duplicate it:
...
wait_for_http "${BASE_URL}/value" 60
...
End the recording in `app.py`. You can now trace object construction, `SET`,
`GET`, and `DEL`, then follow the same code into both Docker topologies.
