...
def test_limit_identity_isolation_ttl_and_expiry(
    ...
    decisions = [limiter.check("identity-a", policy, uuid.uuid7().hex) for _request in range(6)]
    ...
    isolated = limiter.check("identity-b", policy, uuid.uuid7().hex)
    ...
    after_window = limiter.check("identity-a", policy, uuid.uuid7().hex)
