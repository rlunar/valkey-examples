...
run_rate_limiter_demo() {
  local client_a="demo-user-a"
  local client_b="demo-user-b"
  ...
  heading "Start Valkey and ${APP_DISPLAY_NAME}"
  make start
  make reset
  ...
  local implementation_file="src/rate_limiter_demo/valkey/multi_exec.py"
  ...
  if command -v yq >/dev/null; then
    ...
    yq '.id, .status, .valkey.image, .clients, .infrastructure' example.yaml
  fi
  ...
  local window_seconds=$(((RATE_LIMIT_WINDOW_MS + 999) / 1000))
  local recovery_deadline=$(($(date +%s) + (window_seconds * 2) + 5))
  ...
  while ((recovery_attempt < 5)); do
    ...
    sleep "$retry_after"
    send_request \
      "$client_a" "200,429" "identity A recovery attempt ${recovery_attempt}"
